...
public class FirstPersonAIO : MonoBehaviour {
    ...
    public void ChangeSense()
    {
        ...
        mouseSensitivity = loadedPlayerData.sens;
    }
    private void Awake(){
        ...
        #region Look Settings - Update
            ...
            if(Input.touchCount>0 && Input.GetTouch(0).phase == TouchPhase.Moved)
            {
                if(EventSystem.current.IsPointerOverGameObject(Input.GetTouch(0).fingerId))
                    return;
                mouseXInput = Input.GetTouch(0).deltaPosition.x;
                mouseYInput = Input.GetTouch(0).deltaPosition.y;
            }
                         if(targetAngles.y > 180) { targetAngles.y -= 360; followAngles.y -= 360; } else if(targetAngles.y < -180) { targetAngles.y += 360; followAngles.y += 360; }
            if(targetAngles.x > 180) { targetAngles.x -= 360; followAngles.x -= 360; } else if(targetAngles.x < -180) { targetAngles.x += 360; followAngles.x += 360; }
            targetAngles.y += mouseXInput *mouseSensitivity*0.1f;
            if (cameraInputMethod == CameraInputMethod.Traditional){ targetAngles.x += mouseYInput * mouseSensitivity*0.1f ;}
            else {targetAngles.x = 0f;}
            targetAngles.x = Mathf.Clamp(targetAngles.x, -0.5f * verticalRotationRange, 0.5f * verticalRotationRange);
            followAngles = Vector3.SmoothDamp(followAngles, targetAngles, ref followVelocity, (cameraSmoothing)/100);
    ...
    private void FixedUpdate(){
        ...
        #elif UNITY_ANDROID
        bool sprintKey = CrossPlatformInputManager.GetButton("Run");
        if(useStamina){
